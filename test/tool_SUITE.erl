...
init_per_group(registry, Config) ->
    ...
    {ok, Pid} = erl_mcp_server_tool_registry:start_link(),
    unlink(Pid),
...
init_per_group(session_dispatch, Config) ->
    ...
    EchoHandler = fun(Params, _State) ->
        ...
        Content = erl_mcp_protocol_content:text(Msg),
        {ok, #{<<"content">> => [erl_mcp_protocol_content:to_map(Content)]}}
    end,
    ...
    ToolsListHandler = fun(_Params, _SessionState) ->
        Tools = erl_mcp_server_tool_registry:list_tools(),
    ...
    {reply, _} = erl_mcp_server_session:handle_message(Session, InitReq),
...
register_and_list_tools(Config) ->
    ...
    ok = erl_mcp_server_tool_registry:register_tool(Tool1, Handler),
    ok = erl_mcp_server_tool_registry:register_tool(Tool2, Handler),
    Tools = erl_mcp_server_tool_registry:list_tools(),
...
unregister_tool(_Config) ->
    ok = erl_mcp_server_tool_registry:unregister_tool(<<"tool-b">>),
    Tools = erl_mcp_server_tool_registry:list_tools(),
    Names = [T#tool.name || T <- Tools],
...
unregister_nonexistent_fails(_Config) ->
    ?assertEqual({error, not_found},
                 erl_mcp_server_tool_registry:unregister_tool(<<"nope">>)).

lookup_tool(_Config) ->
    {ok, Tool, _Handler} = erl_mcp_server_tool_registry:lookup(<<"tool-a">>),
    ?assertEqual(<<"tool-a">>, Tool#tool.name).

lookup_nonexistent_fails(_Config) ->
    ?assertEqual({error, not_found}, erl_mcp_server_tool_registry:lookup(<<"nope">>)).
...
list_tools_pagination(_Config) ->
    ...
    {Page1, Cursor1} = erl_mcp_server_tool_registry:list_tools(#{cursor => undefined,
                                                       page_size => 3}),
    ?assertEqual(3, length(Page1)),
    ?assertNotEqual(undefined, Cursor1),
    {Page2, _Cursor2} = erl_mcp_server_tool_registry:list_tools(#{cursor => Cursor1,
                                                        page_size => 3}),
...
call_tool_via_session(Config) ->
    ...
    {reply, Reply} = erl_mcp_server_session:handle_message(Session, CallReq),
...
call_nonexistent_tool(Config) ->
    Session = proplists:get_value(session, Config),
    CallReq = erl_mcp_protocol_jsonrpc:request(11, <<"tools/call">>, #{
        <<"name">> => <<"nonexistent">>,
        <<"arguments">> => #{}
    }),
    {reply, Reply} = erl_mcp_server_session:handle_message(Session, CallReq),
