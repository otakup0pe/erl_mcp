...
encode_request_test() ->
    Req = erl_mcp_protocol_jsonrpc:request(1, <<"initialize">>, #{<<"key">> => <<"val">>}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Req),
...
encode_request_string_id_test() ->
    Req = erl_mcp_protocol_jsonrpc:request(<<"abc-123">>, <<"ping">>, #{}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Req),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
    ?assertEqual(<<"abc-123">>, maps:get(<<"id">>, Decoded)).

encode_response_test() ->
    Resp = erl_mcp_protocol_jsonrpc:response(1, #{<<"status">> => <<"ok">>}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Resp),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
    ?assertEqual(<<"2.0">>, maps:get(<<"jsonrpc">>, Decoded)),
    ?assertEqual(1, maps:get(<<"id">>, Decoded)),
...
encode_error_test() ->
    Err = erl_mcp_protocol_jsonrpc:error_response(1, ?METHOD_NOT_FOUND, <<"not found">>),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Err),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
    ?assertEqual(<<"2.0">>, maps:get(<<"jsonrpc">>, Decoded)),
    ?assertEqual(1, maps:get(<<"id">>, Decoded)),
...
encode_error_with_data_test() ->
    Err = erl_mcp_protocol_jsonrpc:error_response(1, ?INTERNAL_ERROR, <<"oops">>,
                                      #{<<"detail">> => <<"stack">>}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Err),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
    ErrObj = maps:get(<<"error">>, Decoded),
...
encode_error_null_id_test() ->
    Err = erl_mcp_protocol_jsonrpc:error_response(null, ?PARSE_ERROR, <<"parse error">>),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Err),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
...
encode_notification_with_params_test() ->
    Notif = erl_mcp_protocol_jsonrpc:notification(<<"notifications/progress">>,
                                      #{<<"token">> => <<"t1">>}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Notif),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
...
decode_request_test() ->
    ...
    {ok, Msg} = erl_mcp_protocol_jsonrpc:decode(Json),
...
decode_rejects_missing_jsonrpc_test() ->
    ...
    ?assertMatch({error, missing_jsonrpc_field}, erl_mcp_protocol_jsonrpc:decode(Json)).

decode_rejects_wrong_version_test() ->
    Json = <<"{\"jsonrpc\":\"1.0\",\"id\":1,\"method\":\"ping\"}">>,
    ?assertMatch({error, {invalid_version, _}}, erl_mcp_protocol_jsonrpc:decode(Json)).

decode_rejects_invalid_json_test() ->
    ?assertMatch({error, _}, erl_mcp_protocol_jsonrpc:decode(<<"not json">>)).
...
encode_batch_test() ->
    Req1 = erl_mcp_protocol_jsonrpc:request(1, <<"ping">>, #{}),
    Req2 = erl_mcp_protocol_jsonrpc:request(2, <<"ping">>, #{}),
    {batch, Msgs} = erl_mcp_protocol_jsonrpc:batch([Req1, Req2]),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode({batch, Msgs}),
    {ok, Decoded} = erl_mcp_protocol_json:decode(Bin),
...
decode_batch_test() ->
    ...
    {ok, {batch, Msgs}} = erl_mcp_protocol_jsonrpc:decode(Json),
    ?assertEqual(2, length(Msgs)),
    ?assertMatch(#jsonrpc_request{id = 1}, hd(Msgs)).

batch_rejects_empty_test() ->
    ?assertEqual({error, empty_batch}, erl_mcp_protocol_jsonrpc:batch([])).

decode_batch_rejects_empty_test() ->
    ?assertMatch({error, empty_batch}, erl_mcp_protocol_jsonrpc:decode(<<"[]">>)).
...
error_code_roundtrip_test_() ->
    ...
    [fun() ->
        Code = erl_mcp_protocol_jsonrpc:error_code(A),
        ?assertEqual(A, erl_mcp_protocol_jsonrpc:error_atom(Code))
     end || A <- Atoms].

error_atom_unknown_test() ->
    ?assertEqual(unknown_error, erl_mcp_protocol_jsonrpc:error_atom(9999)).
...
roundtrip_request_test() ->
    Orig = erl_mcp_protocol_jsonrpc:request(99, <<"tools/list">>, #{<<"cursor">> => <<"abc">>}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Orig),
    {ok, Decoded} = erl_mcp_protocol_jsonrpc:decode(Bin),
...
roundtrip_response_test() ->
    Orig = erl_mcp_protocol_jsonrpc:response(99, #{<<"tools">> => []}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Orig),
    {ok, Decoded} = erl_mcp_protocol_jsonrpc:decode(Bin),
...
roundtrip_notification_test() ->
    Orig = erl_mcp_protocol_jsonrpc:notification(<<"notifications/tools/list_changed">>, #{}),
    {ok, Bin} = erl_mcp_protocol_jsonrpc:encode(Orig),
    {ok, Decoded} = erl_mcp_protocol_jsonrpc:decode(Bin),
