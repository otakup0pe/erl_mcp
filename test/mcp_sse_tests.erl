...
encode_data_only_test() ->
    Event = iolist_to_binary(erl_mcp_protocol_sse:encode_event(<<"hello">>)),
    ?assertEqual(<<"data: hello\n\n">>, Event).

encode_with_id_test() ->
    Event = iolist_to_binary(erl_mcp_protocol_sse:encode_event(<<"1">>, <<"hello">>)),
    ?assertEqual(<<"id: 1\ndata: hello\n\n">>, Event).

encode_with_id_and_type_test() ->
    Event = iolist_to_binary(
        erl_mcp_protocol_sse:encode_event(<<"1">>, <<"message">>, <<"hello">>)),
    ?assertEqual(<<"id: 1\nevent: message\ndata: hello\n\n">>, Event).

encode_multiline_data_test() ->
    Event = iolist_to_binary(
        erl_mcp_protocol_sse:encode_event(<<"line1\nline2\nline3">>)),
    ?assertEqual(<<"data: line1\ndata: line2\ndata: line3\n\n">>, Event).

encode_no_id_with_type_test() ->
    Event = iolist_to_binary(
        erl_mcp_protocol_sse:encode_event(undefined, <<"endpoint">>, <<"data">>)),
...
decode_multiple_events_test() ->
    Stream = <<"data: first\n\ndata: second\n\n">>,
    Events = erl_mcp_protocol_sse:decode_events(Stream),
...
roundtrip_simple_test() ->
    Encoded = iolist_to_binary(erl_mcp_protocol_sse:encode_event(<<"test data">>)),
    [Decoded] = erl_mcp_protocol_sse:decode_events(Encoded),
    ?assertEqual(<<"test data">>, maps:get(data, Decoded)).

roundtrip_with_id_test() ->
    Encoded = iolist_to_binary(
        erl_mcp_protocol_sse:encode_event(<<"evt-1">>, <<"{\"jsonrpc\":\"2.0\"}">>)),
    [Decoded] = erl_mcp_protocol_sse:decode_events(Encoded),
